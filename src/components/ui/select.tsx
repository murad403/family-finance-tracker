...
export function SelectContent({ children, className }: SelectContentProps) {
  ...
  return (
    <motion.div
      ...
      className={`absolute left-0 right-0 z-50 mt-1.5 max-h-60 overflow-y-auto rounded-md border border-global-border bg-cyan-500/90 backdrop-blur-sm p-1 shadow-lg outline-none min-w-32 ${className || ''}`}
    >
...
export function SelectItem({ children, value, className }: SelectItemProps) {
  ...
  return (
    <button
      ...
      className={`relative flex w-full cursor-pointer select-none items-center rounded-md py-2.5 pl-3 pr-9 text-sm text-white outline-none transition-colors hover:bg-white/20 hover:text-zinc-50 active:bg-blue-500 disabled:pointer-events-none disabled:opacity-50 text-left ${
        isSelected ? 'active:bg-blue-500 text-white font-semibold' : ''
      } ${className || ''}`}
    >
